...
plt.figure(figsize=[20, 5])  # 20 inch width x 5 inches height

# PLOT ON LEFT - SIMPLE SCATTER
plt.subplot(1, 2, 1)
sb.regplot(data=fuel_econ, x="year", y="comb", truncate=False)
plt.title("Scatter plot year vs comb")
...
plt.title("Scatter plot year vs comb with jitter in x")
plt.show()
...
plt.title("Scatter plot year vs comb with jitter in x and scatter")
plt.show()
...
plt.title("Scatter plot year vs comb with scatter")
plt.show()
