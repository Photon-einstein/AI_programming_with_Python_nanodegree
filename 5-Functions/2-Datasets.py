...
# Scenario:
# You are responsible for managing the storage capacities of data centers in various regions.
# To optimize resource allocation, you need to calculate the data storage density for each region.
# The storage density is defined as the amount of data (in TB) stored per square kilometer of land area.

# Task:
# Write a function named data_storage_density that takes two arguments, data_amount (in TB) and land_area
# (in square kilometers), and returns the data storage density calculated from those values.
...
if "data_storage_density" not in locals():
    ...
    print(explanation_str.format("data_storage_density(10, 1)", 10.0 / 1))
elif data_storage_density(864816, 121.4) != 864816 / 121.4:
    print(
        explanation_str.format("data_storage_density(864816, 121.4)", 864816.0 / 121.4)
    )
elif data_storage_density(1234321, 42) != 1234321 / 42:
    print(explanation_str.format("data_storage_density(1234321, 42)", 1234321.0 / 42))
else:
    print("Nicely done! You can view my solution on the next page.")

# Exercise 2: Readable Time for Data Processing
# Scenario:
# You are optimizing a machine learning pipeline, and you need to report the processing
# time of a dataset in a human-readable format. The time is provided in days, and you need
# to convert it to weeks and days to better communicate the processing schedule to the
# stakeholders.
